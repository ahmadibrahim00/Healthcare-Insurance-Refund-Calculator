...
public class Validation {
     ...
     * @return un booleen signifiant la presenece des cles ou non.
     */
     ...
     * @return un booleen validant ou non la longueur du numero du client.
     */
    private boolean verifierLongueurNumeroClient(){
        String numeroClient;
        if (validerLaPresenceDesCles()){
    ...
    private void estNumeroCLientValide () {
        if(verifierLongueurNumeroClient()) {
            try {
     ...
     * @return un boolean validant ou non le numero du client.
     */
     ...
     * @return un booleen validant ou non que le mois et l'annee sont composes seulement de chiffres.
     */
    private boolean validerTypeMois() {
        if(estFormatMoisValide()) {
            String mois = String.valueOf(reclamationClient.get("mois"));
            try {
            ...
            if (!valide || Long.parseLong(mois.substring(0, 4)) < 0 || Long.parseLong(mois.substring(0, 4)) < 1970)
     ...
     * @return un boolean validant la valeur de la cle "mois".
     */
     ...
     * @return valide, un booleen verifiant la validite des cles presentes dans le JSONArray "reclamations".
     */
    ...
    private String[] obtenirValeur(String cle) {
        String[] tabVal = null;
        if (validerLaPresenceDesClesRecalmations(obtenirReclamations())) {
     ...
     * @return valide un booleen verifiant l'egalite.
     */
    private boolean verifierEgaliteMoisDate() {
        if (estMoisValide() && verifieLongueurDate()) {
            int position = 0;
            while (valide && position < obtenirValeur("mois").length) {
     ...
     * @return un booleen verifier le format des dates.
     */
    private boolean validerFormatDate () {
        if (verifierEgaliteMoisDate()){
            int position = 0;
     ...
     * type long.
     * @return un boolean validant  que le type des jours.
     */
    private boolean validerTypeJours() {
        if(validerFormatDate()) {
            try {
     ...
     * @return un booleen validant ou non la presence d signe de $.
     */
     ...
     * @return un booleen verifiant que les montant sont de type float.
     */
    ...
    /**
     * Cette methode permet de valider que les montants entres sont valides.
     * */
    private void estMontantValide() {
        if (validerTypeMontant()) {
            String [] montant = obtenirValeur("montant");
